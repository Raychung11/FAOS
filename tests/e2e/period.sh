...
ckc "$(curl -s -b /tmp/pe_a.txt $AH -X POST $B/api/finance/periods/close -d '{"period_start":"'$PREVMONTH_START'","period_end":"'$PREVMONTH_END'","note":"Previous month"}')" '"status":"closed"' "close previous month"
ckc "$(curl -s -b /tmp/pe_a.txt $B/api/finance/periods)" '"period_start":"'$PREVMONTH_START'"' "period listed as closed"
...
ckc "$(curl -s -b /tmp/pe_w.txt $WH -X POST $B/api/sales -d '{"payment_type":"cash","client_uuid":"PER-BACK","sold_at":"'$PREVMONTH_MID' 09:00:00","items":[{"product_id":1,"qty":1,"unit_price":9.90}]}')" 'is closed' "backdated sale into closed period rejected"
...
ckc "$(curl -s -b /tmp/pe_a.txt $AH -X POST $B/api/reconciliation/import -d '{"outlet_id":1,"period_start":"'$PREVMONTH_START'","period_end":"'$PREVMONTH_END'","source_name":"AEON","lines":[{"sku":"CF-LATTE","qty":3,"amount":29.7}]}')" '"variance"' "delayed reconciliation NOT blocked by closed period"

# Close the CURRENT period (covers today).
curl -s -b /tmp/pe_a.txt $AH -X POST $B/api/finance/periods/close -d '{"period_start":"'$CURMONTH_START'","period_end":"'$CURMONTH_END'","note":"Current month"}' >/dev/null
MID=$(dbq "SELECT id FROM accounting_periods WHERE period_start='$CURMONTH_START' AND company_id=1")
