<?php
/** AdvisorOS — Tax Planning Knowledge Base (educate the AI). */
...
if (is_post()) {
    ...
    if ($action === 'delete') {
        tax_kb_delete((int) input('id', 0));
        audit_log('delete', 'tax_kb', (int) input('id', 0), 'Tax knowledge entry removed');
        set_flash('success', 'Entry removed.');
        redirect('advisor/tax-knowledge.php');
    }
    if ($action === 'toggle') {
        $e = tax_kb_get((int) input('id', 0));
        if ($e) {
            tax_kb_save((int) $e['id'], (string) $e['title'], (string) ($e['category'] ?? ''),
    ...
    if ($title === '' || $body === '') {
        ...
        redirect('advisor/tax-knowledge.php' . (($id = (int) input('id', 0)) ? '?edit=' . $id : ''));
    }
    $newId = tax_kb_save((int) input('id', 0), $title, (string) input('category', ''),
        $body, input('active', '') !== '');
    audit_log((int) input('id', 0) ? 'update' : 'create', 'tax_kb', $newId, 'Tax knowledge saved');
    set_flash('success', 'Knowledge entry saved.');
    redirect('advisor/tax-knowledge.php');
}

$entries = tax_kb_all();
$editing = $editId ? tax_kb_get($editId) : null;

$pageTitle = 'Tax Knowledge Base';
require __DIR__ . '/../includes/header.php';
?>
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;flex-wrap:wrap;gap:10px">
  <div>
    <h2 style="margin:0;color:var(--navy)">Tax Knowledge Base</h2>
    <span class="muted">Paste your tax methodology — the AI uses active entries when writing tax reports</span>
  </div>
</div>
...
<div class="grid cols-2">
  <div class="card-os">
    <div class="card-os-head"><?= $editing ? 'Edit entry' : 'Add knowledge' ?>
      <?php if ($editing): ?>
        <a class="btn-os ghost sm" href="<?= e(url('advisor/tax-knowledge.php')) ?>">Cancel</a>
      <?php endif; ?>
    </div>
      ...
      <form method="post">
        ...
        <div class="form-row"><label>Title *</label>
          <input name="title" required maxlength="140" value="<?= e($editing['title'] ?? '') ?>"
                 placeholder="e.g. Relief maximisation rules · Salary vs dividend policy"></div>
        <div class="form-row"><label>Category</label>
          <input name="category" maxlength="50" value="<?= e($editing['category'] ?? '') ?>"
                 placeholder="Reliefs / Employment / Business / Traps / House style"></div>
        <div class="form-row"><label>Content * <span class="muted" style="font-weight:400">(paste from your documents — methodology, rules, traps, house style)</span></label>
          <textarea name="body" rows="14" required maxlength="12000"
            placeholder="Paste the guidance you want the AI to follow when producing tax planning reports…"><?= e($editing['body'] ?? '') ?></textarea></div>
        <label style="display:flex;gap:8px;align-items:center;margin:6px 0 14px">
          ...
          <span>Active — feed this to the AI tax report</span></label>
        <button class="btn-os gold"><?= $editing ? 'Save entry' : 'Add entry' ?></button>
      </form>
      <div class="disclaimer">Tax figures (rates, relief caps, the computation)
        come from the system's verified engine, not the AI. The knowledge base
        steers the AI's <em>narrative and planning advice</em> only.</div>
    </div>
  </div>
  ...
  <div class="card-os">
    ...
    <div class="card-os-body" style="padding:0">
      <?php if (!$entries): ?>
        <div style="padding:18px" class="muted">No entries yet. Paste your tax
          methodology on the left — each active entry becomes context for the AI
          when it writes a client's tax report.</div>
      <?php else: ?>
          ...
          <?php foreach ($entries as $e): ?>
            <tr>
              ...
              <td style="text-align:right;white-space:nowrap">
                <a class="btn-os ghost sm" href="<?= e(url('advisor/tax-knowledge.php?edit='.(int)$e['id'])) ?>">Edit</a>
